...
class vector{
    public static void main(String[] args)
    {
        ...
        int preve[][] = new int[n][10];
        for ( int i = 0; i<n;i++)
        {
            System.out.print("Enter no. of events of Process "+(i+1)+": ");
            ne[i] = sc.nextInt();
            System.out.println("Enter 0 for Internal, 1 for Send, 2 for Receive");
            for (int j = 0; j < ne[i]; j++)
            {
              
                if((status[i][j] != 1) && (status[i][j] !=2))
                {
                    System.out.print("Event "+(j+1)+": ");
                    status[i][j]= sc.nextInt();
                    if(status[i][j] == 1)
                    {
                        System.out.print("Enter Receiving Process: ");
                        int a = sc.nextInt();
                        System.out.print("Enter Receiving Event: ");
                        int b = sc.nextInt();
                        status[(a-1)][(b-1)] = 2;
                    ...
                    {
                        System.out.print("Enter Sending Process: ");
                        int a = sc.nextInt();
                        System.out.print("Enter Sending Event: ");
                        int b = sc.nextInt();
                        status[(a-1)][(b-1)] = 1;
        ...
        for(int x = n-1; x>-1;x--)
        {
            for(int z=0;z<ne[x];z++)
            {
                ...
                { 
                    if ( x == n-1)
                    {
                        int sum = ((time[(prevp[x][z])][(preve[x][z])])/10);
                        sum = sum*10;
                        time[x][z] = sum + time[x][z];
                    }
                    else if( x == 0 ) {
                        int sum = ((time[(prevp[x][z])][(preve[x][z])])%p);
                        time[x][z] = sum + time[x][z];
                    }
                    else
        ...
        for(int x1 =n-1; x1 > -1; x1--)
        {
            for(int z1 =0;z1 <ne[x1]; z1 ++)
            {
                ...
                if(status[x1][z1] == 2)
                {
                    if ( x1 == n-1)
                    {
                        int sum = ((time[(prevp[x1][z1])][(preve[x1][z1])])/10);
                        sum = sum*10;
                        time[x1][z1] = sum + time[x1][z1];
                    }
                    else if( x1 == 0 ) {
                        int sum = ((time[(prevp[x1][z1])][(preve[x1][z1])])%p);
                        time[x1][z1] = sum + time[x1][z1];
                    }
                    else
        ...
        for(int q =0; q<n;q++)
        {
            System.out.print("Process "+(q+1)+"\t");
            for(int p=0;p<ne[q];p++)
            {
                String format  = ("%0"+n+"d");
